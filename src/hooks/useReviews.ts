...
interface ListResp { items?: Review[]; data?: Review[]; reviews?: Review[] }
...
export const useProductReviews = (productId?: string) =>
  useQuery({
    ...
    enabled: !!productId,
    queryFn: async () => unwrap((await api.get<ListResp | Review[]>(`/products/${productId}/reviews`)).data),
  });
...
export const useProductRating = (productId?: string) =>
  useQuery({
    ...
    enabled: !!productId,
    queryFn: async () => {
      const { data } = await api.get<ProductRatingSummary>(`/products/${productId}/reviews/summary`);
...
export const useReplyToReview = (productId?: string) => {
  ...
  return useMutation({
    mutationFn: async (input: { reviewId: string; body: string }) => {
      const { data } = await api.post(`/reviews/${input.reviewId}/reply`, { body: input.body });
      return data as Review;
    },
