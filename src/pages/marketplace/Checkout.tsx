...
import { Bike, Loader2, Package, ShieldCheck, Store, Truck } from "lucide-react";
...
const METHODS: { id: Method; label: string; desc: string; fee: number; icon: React.ElementType }[] = [
  ...
  { id: "express", label: "Express delivery", desc: "Same-day where available", fee: 3500, icon: Truck },
  { id: "pickup", label: "Farm pickup", desc: "Collect directly from the farmer", fee: 0, icon: Store },
];
...
const Checkout = () => {
  ...
  const [addressId, setAddressId] = useState<string | undefined>(defaultAddress?.id);
  ...
  if (items.length === 0) {
    return (
      <div className="container py-16 text-center">
        ...
        <Button className="mt-4 rounded-full" onClick={() => navigate("/marketplace")}>Browse products</Button>
      </div>
    );
  }
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      else { toast.success("Order placed"); navigate("/marketplace"); }
    } catch (err) {
  ...
  return (
    <div className="container py-8">
      ...
      <p className="text-sm text-muted-foreground">Confirm your delivery details and pay securely.</p>

      <div className="mt-6 grid gap-8 lg:grid-cols-[1fr_380px]">
        <div className="space-y-8">
          <section className="rounded-2xl border border-border bg-card p-5 shadow-card">
            <AddressBook selectable selectedId={addressId} onSelect={setAddressId} />
          </section>
                ...
                return (
                  ...
                  >
                    <span className={cn(
                      ...
                      active ? "bg-gradient-primary text-white shadow-glow" : "bg-secondary text-primary",
                    )}>
              ...
              {items.map((i) => (
                <li key={i.productId} className="flex items-center gap-3 py-3">
                  <div className="h-12 w-12 overflow-hidden rounded-lg bg-secondary">
                    {i.image ? <img src={i.image} alt="" className="h-full w-full object-cover" /> : <Package className="m-3 h-6 w-6 text-muted-foreground" />}
                  </div>
                  <div className="flex-1">
                    <p className="text-sm font-semibold">{i.title}</p>
                    <p className="text-xs text-muted-foreground">Qty {i.quantity}{i.unit ? ` · ${i.unit}` : ""}</p>
                  </div>
                  <p className="text-sm font-bold">{formatNaira(i.price * i.quantity)}</p>
                </li>
              ))}
            </ul>
          </section>
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <div className="rounded-2xl border border-border bg-card p-6 shadow-card">
            ...
            <dl className="mt-4 space-y-2 text-sm">
              ...
              <Row label="Delivery" value={method === "pickup" ? "Free" : formatNaira(selectedFee)} />
            ...
            <p className="mt-3 flex items-center justify-center gap-1 text-xs text-muted-foreground">
              <ShieldCheck className="h-3 w-3 text-primary" /> Secure escrow payment via Paystack
            </p>
          </div>
        </aside>
      </div>
