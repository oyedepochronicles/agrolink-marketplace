...
import { Camera, HelpCircle, Loader2, Lock, LogOut, Mail, Phone, ShieldAlert, ShieldCheck } from "lucide-react";
...
const Profile = () => {
  ...
  const onPick = async (file?: File) => {
    if (!file) return;
    if (file.size > 5 * 1024 * 1024) { toast.error("Avatar must be ≤5MB"); return; }
      ...
      <div className="overflow-hidden rounded-3xl border border-border bg-card shadow-card">
        <div className="bg-gradient-hero p-8 text-white">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-2">
                {isVerified ? (
                  <Badge className="border-0 bg-white/15 text-white"><ShieldCheck className="mr-1 h-3 w-3" /> Verified</Badge>
                ) : isPending ? (
                  <Badge className="border-0 bg-warning/30 text-white"><Loader2 className="mr-1 h-3 w-3 animate-spin" /> Verification pending</Badge>
                ) : (
                  <Badge className="border-0 bg-white/15 text-white"><ShieldAlert className="mr-1 h-3 w-3" /> Not verified</Badge>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="space-y-1 p-8">
          <div className="mb-3 flex items-center gap-2 text-xs text-muted-foreground">
            <Lock className="h-3 w-3" /> Account details are locked. Contact support to make changes.
          </div>
          ...
          <div className="flex flex-wrap items-center justify-end gap-2 pt-6">
            <Button variant="outline" className="rounded-full" asChild>
              <Link to="/marketplace/support"><HelpCircle className="mr-2 h-4 w-4" /> Contact support</Link>
            </Button>
            {!isVerified && !isPending && (
              <VerificationRequestDialog
                trigger={<Button className="rounded-full bg-gradient-primary shadow-glow"><ShieldCheck className="mr-2 h-4 w-4" /> Request verification</Button>}
              />
            )}
