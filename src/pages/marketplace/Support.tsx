...
const Support = () => {
  ...
  const submit = async () => {
    ...
    if (!s || s.length < 4) { toast.error("Subject is too short"); return; }
    if (!b || b.length < 10) { toast.error("Add more detail to your message"); return; }
    try {
  ...
  return (
    <div className="container max-w-4xl space-y-10 py-10">
      <header className="space-y-3 text-center">
        ...
        <h1 className="font-display text-3xl font-extrabold md:text-4xl">How can we help?</h1>
        <p className="text-sm text-muted-foreground">Search our knowledge base or open a ticket — we usually reply within a few hours.</p>
        <div className="relative mx-auto max-w-xl">
          <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search the knowledge base..." className="h-11 rounded-full bg-secondary pl-10" />
        </div>
      </header>

      <section className="space-y-3">
        <h2 className="font-display text-xl font-extrabold">Frequently asked</h2>
        {faqLoading ? (
          <div className="space-y-2">{[1,2,3].map((i) => <Skeleton key={i} className="h-12 w-full rounded-xl" />)}</div>
        ) : filtered.length === 0 ? (
          <Card className="rounded-2xl p-6 text-center text-sm text-muted-foreground">No matching articles. Open a ticket below.</Card>
        ) : (
          <Accordion type="single" collapsible className="w-full rounded-2xl border border-border bg-card px-2">
            {filtered.map((f) => (
              <AccordionItem key={f._id} value={f._id} className="px-2">
                ...
                <AccordionContent className="whitespace-pre-wrap text-sm text-foreground/80">{f.answer}</AccordionContent>
              </AccordionItem>
            ))}
          </Accordion>
        )}
      ...
      <section className="space-y-3">
          ...
          {user && (
            <Dialog open={open} onOpenChange={setOpen}>
              <DialogTrigger asChild>
                <Button className="rounded-full"><Plus className="mr-1 h-4 w-4" /> New ticket</Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader><DialogTitle>Open a support ticket</DialogTitle></DialogHeader>
                <div className="space-y-3">
                  <Input placeholder="Subject" value={subject} onChange={(e) => setSubject(e.target.value)} maxLength={120} />
                  <Textarea placeholder="Describe your issue..." rows={5} value={body} onChange={(e) => setBody(e.target.value)} maxLength={2000} />
                </div>
                <DialogFooter>
        ...
        {!user ? (
          <Card className="rounded-2xl p-6 text-center text-sm text-muted-foreground">
            <Link to="/login" className="font-semibold text-primary">Sign in</Link> to open a support ticket.
          </Card>
        ) : ticketsLoading ? (
          <div className="space-y-2">{[1,2].map((i) => <Skeleton key={i} className="h-16 w-full rounded-xl" />)}</div>
        ) : tickets.length === 0 ? (
          <Card className="rounded-2xl p-6 text-center text-sm text-muted-foreground">No tickets yet — questions are welcome anytime.</Card>
        ) : (
          <div className="space-y-2">
            {tickets.map((t) => (
              <Link key={t._id} to={`/marketplace/support/${t._id}`} className="block">
                <Card className="rounded-2xl p-4 transition-base hover:border-primary/50 hover:shadow-glow">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="line-clamp-1 text-xs text-muted-foreground">{t.body}</p>
                    </div>
                    <div className="text-right">
                      <Badge variant="outline" className={STATUS_TONE[t.status]}>{t.status}</Badge>
                      <p className="mt-1 text-[10px] text-muted-foreground">{formatDate(t.updatedAt)}</p>
                    </div>
                  </div>
                </Card>
              </Link>
