  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <StatCard icon={<Box className="h-4 w-4" />} label="Active products" value={String(cards.activeProducts ?? "-")} hint={`${cards.totalProducts ?? 0} total listings`} />
        <StatCard icon={<ShoppingCart className="h-4 w-4" />} label="Orders this month" value={String(cards.ordersThisMonth ?? "-")} hint={`${cards.pendingOrders ?? 0} pending`} />
        <StatCard icon={<Wallet className="h-4 w-4" />} label="Wallet balance" value={formatNaira(cards.walletBalance ?? 0)} hint={`${formatNaira(cards.grossSales ?? 0)} gross sales`} />
      </div>

      <div className="grid gap-4 lg:grid-cols-[2fr_1fr]">
        <Card className="rounded-2xl p-5 shadow-card">
          <h3 className="font-semibold">Sales trend</h3>
          <ChartContainer config={{ revenue: { label: "Revenue", color: "hsl(var(--primary))" } }} className="mt-4 h-64">
