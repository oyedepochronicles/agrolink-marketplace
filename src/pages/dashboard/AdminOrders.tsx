...
const AdminOrders = () => {
  ...
  const columns = useMemo<ColumnDef<Row>[]>(() => [
    { accessorKey: "shortId", header: t("admin.columns.orderId"), cell: ({ row }) => <span className="font-mono text-xs font-semibold">{row.original.shortId}</span> },
    { accessorKey: "product", header: t("admin.columns.product"), cell: ({ row }) => <span className="font-medium">{row.original.product}</span> },
    { accessorKey: "buyer", header: t("admin.columns.buyer") },
    { accessorKey: "farmer", header: t("admin.columns.farmer") },
    { accessorKey: "amount", header: t("admin.columns.amount"), cell: ({ row }) => <span className="font-semibold">{formatNaira(row.original.amount)}</span> },
    { accessorKey: "payment", header: t("admin.columns.payment"), cell: ({ row }) => (
      <Badge variant="outline" className={row.original.payment === "paid" ? "border-primary/30 bg-primary/10 text-primary" : "border-warning/30 bg-warning/10 text-warning-foreground"}>
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t("admin.ordersTitle")} description={t("admin.ordersDesc")} />
      {isLoading ? (
        <div className="flex justify-center py-16"><Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /></div>
      ) : rows.length === 0 ? (
        <EmptyState icon={<ShoppingCart className="h-6 w-6" />} title={t("common.noResults")} />
      ) : (
        <DataTable
          data={rows}
          columns={columns}
